...
本包已接入云端服务 `https://relationship-access.jp1996.workers.dev`，目前供首轮顾客流程验收使用。服务地址已内置，不需要顾客填写；原计算入口已关闭，不要替换为旧地址。当前商品为独立1000次卡，不设到期时间；以激活后的查询结果为准。
...
购买页面： https://payhip.com/b/pFVqj 。AI只提供购买入口，不代付款。新码目前是独立次数卡，不自动叠加，也不是订阅。用完后新卡应保存在新的Secret目录，并让AI在后续计算中使用新目录；旧案例恢复仍必须使用原目录，不覆盖旧凭证。
